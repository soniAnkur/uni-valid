...
export class ValidationType {
    ...
    static NUMBER = "MANDATORY"; //meta for min/mas is requied
    static STRING = "MANDATORY"; //meta for min/mas is requied 
}
...
export class ValidationHelper {
    ...
    private static checkMandatory(rule: ValidationRule, val: any): ValidationRule {
        rule.valid = (val && val !== '');
        return rule;
    }
    private static checkString(rule: ValidationRule, val: any): ValidationRule {
        rule.valid = (val.length >= rule.meta[MetadataKeys.MIN_LENGTH] && val.length <= rule.meta[MetadataKeys.MIN_LENGTH]);
        return rule;
    }
    private static checkRegExp(rule: ValidationRule, val: any): ValidationRule {
        return rule;
    }
}
